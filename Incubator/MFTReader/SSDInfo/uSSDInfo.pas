...
uses Windows, uDiskFunctions, uAlert, Classes, Math, Dialogs, SysUtils;
...
begin
  ...
  if (Pos('LITEONIT', Model) > 0) or
      (Pos('PLEXTOR', Model) > 0) or
      (Pos('Ninja-', Model) > 0) or
      (EnableSamsung and (Pos('SAMSUNG', Model) > 0) and (Pos('830', Model) > 0)) or
      ...
      ((Pos('HYNIX', Model) > 0) and (Pos('HFS', Model) > 0)) then
  ...
  begin
    NativeMax := ReadNativeMaxAddress(Harddisk);
    if UserSize = NativeMax + 1 then
      NativeEqualSize := true
    else
      NativeEqualSize := false;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ICBuffer.Buffer[85 * 2] and Ceil(Power(2, 10))) >= 1 then
        HPAEnabled := true;
    end;
    CloseHandle(hdrive);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ICBuffer.Buffer[82] and Ceil(Power(2, 10))) >= 1 then
        HPAEnabled := true;
    end;
    CloseHandle(hdrive);
  end;
...
begin
  ...
  if Pos('SAMSUNG', Model) > 0 then HostWrites := round(ExtractSMART(SMARTData, 'F1') / 1024 / 2048 * 10 * 1.56)
          ...
          (Pos('SHYSF', Model) > 0) or (Pos('Patriot Pyro', Model) > 0) or
          ((Pos('SuperSSpeed', Model) > 0) and (Pos('Hyper', Model) > 0)) or
          ((Pos('HYNIX', Model) > 0) and (Pos('HFS', Model) > 0)) then HostWrites := ExtractSMART(SMARTData, 'F1') * 16
  else if (Pos('Ninja-', Model) > 0) or (Pos('M5P', Model) > 0) then HostWrites := (ExtractSMART(SMARTData, 177) * 2)
  else if (Pos('M4', Model) > 0) and (Pos('CT', Model) > 0) then HostWrites := 0
  else HostWrites := ExtractSMART(SMARTData, 177);

  if Pos('SAMSUNG', Model) > 0 then EraseError := ExtractSMART(SMARTData, 'B6')
  else if (Pos('MX', Model) > 0) and (Pos('MMY', Model) > 0) then EraseError := ExtractSMART(SMARTData, 1)
  else if (Pos('MXSSD', Model) > 0) or ((Pos('OCZ', Model) > 0) and (Pos('VERTEX3', Model) > 0)) or
          ((Pos('OCZ', Model) > 0) and (Pos('AGILITY3', Model) > 0)) or ((Model = 'SSD 128GB') or (Model = 'SSD 64GB')) or
          (Pos('SHYSF', Model) > 0) or (Pos('Patriot Pyro', Model) > 0) or
          ((Pos('SuperSSpeed', Model) > 0) and (Pos('Hyper', Model) > 0)) or
          ((Pos('HYNIX', Model) > 0) and (Pos('HFS', Model) > 0)) then EraseError := ExtractSMART(SMARTData, 'AC')
