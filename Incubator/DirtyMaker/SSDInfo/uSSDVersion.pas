...
begin
  if (Pos('MX', Model) > 0) and (Pos('DSF', Model) > 0) and (StrToInt(Revision[1] + Revision[3] + Revision[5])
                                                          >= StrToInt(LastDS[1] + LastDS[3] + LastDS[5])) and
                                                          (Length(Revision) = 5) then
    result := NEW_VERSION
...
begin
  ...
  if (Model = 'OCZ-VERTEX3') or (Model = 'OCZ-AGILITY3') then Result := FloatToStr(LastVerAgi3)

  else if (Pos('MX', Model) > 0) and (Pos('DS', Model) > 0) then Result := LastDS

  else if Copy(Revision, 1, 3) = 'VA8' then Result := Result + IntToStr(LastVA8)
