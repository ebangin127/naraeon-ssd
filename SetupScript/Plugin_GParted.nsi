...
!define PRODUCT_NAME "Naraeon SSD Tools - Eraser Pack(GParted)"
...
Name "Naraeon SSD Tools - Eraser Pack(GParted)"
OutFile "..\Setup\NST(GParted)_New.exe"
...
Section "MainSection" SEC01
  ...
  File "..\exec_files\gparted.iso"
  SetOutPath "$INSTDIR\SSDTools\Unetbootin"
  File "..\exec_files\unetbootin.exe"
SectionEnd

Section -AdditionalIcons
  CreateShortCut "$SMPROGRAMS\Naraeon SSD Tools\Uninstall(GParted).lnk" "$INSTDIR\SSDTools\uninst_plugin_gparted.exe"
SectionEnd

Section -Post
  WriteUninstaller "$INSTDIR\SSDTools\uninst_plugin_gparted.exe"
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\SSDTools.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\SSDTools\uninst_plugin_gparted.exe"
...
Section Uninstall
  Delete "$INSTDIR\Erase\gparted.iso"
  Delete "$INSTDIR\uninst_plugin_gparted.exe"
